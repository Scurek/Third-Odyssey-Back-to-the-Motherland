...
{%- if "province_list_override" not in road %}
...
{%- if "province_list_override" not in road %}
to_elysian_highway_{{ road["name"] }}_trigger = {
    # Code generated by {{ file_path }}
    [[ANY]OR = {]
    {%- for province in road["provinces"] %}
    {{ province["id"] }} = { $TRIGGER$ }
...
{%- if "flag_override" not in road and "province_list_override" not in road %}
to_elysian_highway_{{ road["name"] }}_effect = {
    # Code generated by {{ file_path }}
{%- for province in road["provinces"] %}
    if = {
        limit = { {{ province["id"] }} = { $PROVINCE_TRIGGER$ } }
        {{ province["id"] }} = { $PROVINCE_EFFECT$ }
    }
{%- endfor %}
}
{%- endif %}
...
templateClearBusyFlags = Template('''to_clear_highway_busy_flags = {
    # Code generated by {{ file_path }}
	{%- for road in roads %}
	{%- if "flag_override" not in road and "has_mission_trigger" not in road %}
	clr_province_flag = to_local_elysian_highway_busy_{{ road["name"] }}
	{%- endif %}
	{%- endfor %}
}
''')

templateTriggerEvent = Template('''to_trigger_elysian_highway_{{ road["name"] }}_finish_event = {
    # Code generated by {{ file_path }}
    country_event =  { id = {{ road["event"] }} }
}
''')

f = open(output_file_scripted_effects, "w")
f.write(templateClearBusyFlags.render(roads=roads, file_path=file_path))
f.write("\n")
f.write(template.render(roads=roads, file_path=file_path))
f.write("\n")
for road in roads:
    if "event" in road:
        f.write(templateTriggerEvent.render(road=road, file_path=file_path))
        f.write("\n")
f.close()
...
defined_text = {
	...
	text = {
		...
		trigger = {
			{{ province["id"] }} = { is_city = yes country_or_subject_or_ally_or_ally_subject_holds = { TAG = ROOT } }
		}
	}
	text = {
...
templateCurrentTarget = Template('''defined_text = {
	...
	text = {
		trigger = {
		    has_province_flag = to_local_elysian_highway_busy_{{ road["name"] }}
		}
		localisation_key = to_elysian_highway_construction_target_{{ road["name"] }}_tt
	}
	{%- endif %}
	{%- endfor %}
}
...
f.write(templateCurrentTarget.render(roads=roads))
f.write("\n")
...
f.write(f"# Code generated by {os.path.basename(os.getcwd())}/{os.path.basename(__file__)}\n")
for road in roads:
    if "has_mission_trigger" not in road or "province_list_override" in road:
        continue
    f.write(f' to_owns_elysian_highway_{road["name"]}_tt:0 "The following provinces are fully owned by §Y['
            f'Root.GetName]§! or '
...
f.write('\n')
for road in roads:
    if "has_mission_trigger" in road or "province_list_override" in road:
        continue
    f.write(f' to_owns_elysian_highway_{road["name"]}_tt:0 "At least one of the following provinces is fully owned '
...
# f.write('\n')
for road in roads:
    if "flag_override" in road or "has_mission_trigger" in road:
        continue
    f.write(f' to_elysian_highway_construction_target_{road["name"]}_tt:0 \"')
    f.write(f'[to_elysian_highway_{road["name"]}_current.GetName]\"\n')
f.write('\n')
for road in roads:
    if "no_decision" in road or "province_list_override" in road:
        continue
...
{%- if "no_decision" not in road %}
    to_construct_{{ road["name"] }}_elysian_highway = {
		potential = {
			...
			{%- if "flag_override" in road %}
			NOT = { has_global_flag = to_elysian_highways_{{ road["flag_override"] }}_enabled }
			{%- else %}
			NOT = { has_global_flag = to_elysian_highways_{{ road["name"] }}_enabled }
			{%- endif %}
			{%- if "additional_potential" in road %}
			{{ road["additional_potential"] }}
			{%- endif %}
		}
		...
		provinces_to_highlight = {
		    ...
		    OR = {
		        {%- if "province_list_override" in road %}
                to_elysian_highway_{{ road["province_list_override"] }}_province_trigger = { TRIGGER = province_id }
                {%- else %}
		...
		allow = {
			...
			custom_trigger_tooltip = {
			    {%- if "province_list_override" in road %}
				tooltip = to_owns_elysian_highway_{{ road["province_list_override"] }}_tt
                {%- else %}
		...
		effect = {
			...
			{%- if "flag_override" in road %}
			set_global_flag = to_elysian_highways_{{ road["flag_override"] }}_enabled
			{%- else %}
			set_global_flag = to_elysian_highways_{{ road["name"] }}_enabled
			{%- endif %}
			{%- if "province_list_override" in road %}
			custom_tooltip = to_elysian_highway_{{ road["province_list_override"] }}_provinces_effect_list_tt
			{%- else %}
