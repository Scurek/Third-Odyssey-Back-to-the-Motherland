...
with open('Compatibility - List1.tsv') as file:
    ...
    for i, row in enumerate(rd):
        if i == 0:
            continue
        if len(row) < 3 or row[COMPATIBILITY_COLUMN] not in ['yes', 'no', 'requires patch']:
            continue
        mod_id = parse_id(row[URL_COLUMN])
        mod_name = row[0].strip()
        if row[COMPATIBILITY_COLUMN] != 'yes':
            ...
            if len(row) >= COMPATIBILITY_PATCH_URL + 1 and row[COMPATIBILITY_PATCH] != "":
                ...
                compatibility_patch_entry = {"name": row[COMPATIBILITY_PATCH],
                                             "mod_id": compatibility_patch_id,
                                             "parent_mod": mod_name}
                incompatible_mod_entry["compatibility_patch"] = compatibility_patch_entry
                compatibility_patches.append(compatibility_patch_entry)
...
with open(output_file_loc, "w", encoding="utf-8-sig") as file_loc:
    ...
    for mod in incompatible_mods:
        ...
        if "compatibility_patch" in mod:
            file_loc.write(
                f" to_mod_{mod['mod_id']}_compatibility_tt:0 \"{mod_name} §g(With \\\"{parse_mod_name(mod["compatibility_patch"]["name"])}\\\")§!\"\n")
...
with open(output_file_events, "w") as file_events:
    ...
    file_events.write(template.render(incompatible_mods=incompatible_mods, compatible_mods=compatible_mods, compatibility_patches=compatibility_patches))
