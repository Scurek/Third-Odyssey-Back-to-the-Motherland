...
for exarch in exarchs:
    f.write(f"to_{exarch["tag"]}_exarchs_all_provinces_with_low_liberty_desire = {{\n")
    f.write(f"	# Code generated by {os.path.basename(os.getcwd())}/{os.path.basename(__file__)}\n")
    f.write(f"	{exarch["reference_target"]} = {{ to_exarchs_all_provinces_with_low_liberty_desire = {{ EXARCH_TAG = {exarch["tag"]} }} }}\n")
    f.write("}\n\n")

f.close()


output_file = "o_just_a_little_exarchise_triggers.txt"
f = open(output_file, "w", encoding="utf-8")
for i, exarch in enumerate(exarchs):
    f.write("      {\n")
    f.write(f"        \"trigger\": \"to_{exarch["tag"]}_exarchs_all_provinces_with_low_liberty_desire\",\n")
    f.write(f"        \"text\": \"§Y[{exarch["tag"]}.GetName]§! meets the achievement conditions.\"\n")
    f.write("      }")
    if i != len(exarchs) - 1:
        f.write(",\n")
f.close()
