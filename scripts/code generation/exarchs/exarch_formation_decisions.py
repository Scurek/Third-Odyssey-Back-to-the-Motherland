...
for exarch in exarchs_filtered:
    ...
    if 'merchant_land2' in exarch:
        merchant_land = (exarch['merchant_land2'].replace("[H]", HIGHLIGHT).replace("[!H]", HIGHLIGHT_STOP)
                         .replace("[A]", ANTI_HIGHLIGHT).replace("[!A]", ANTI_HIGHLIGHT_STOP))
        f.write(f" to_exarch_merchant_land_2_{exarch['tag']}_tt:0 \"{merchant_land}\"\n")
...
f.write("defined_text = {\n")
f.write(f"	# Code generated by {os.path.basename(os.getcwd())}/{os.path.basename(__file__)}\n")
f.write("	name = GetExarchMerchantLand_2\n"
        "	random = no\n\n")
for exarch in exarchs_filtered:
    ...
    f.write(f"	text = {{\n"
            f"		localisation_key = to_exarch_merchant_land_2_{exarch['tag']}_tt\n"
            f"		trigger = {{ tag = {exarch['reference_target']} }}\n"
            f"	}}\n")
f.write("}\n\n")
...
for exarch in exarchs_filtered:
    if "merchant_land2" in exarch:
        f.write(f"	text = {{\n"
                f"		localisation_key = to_exarch_merchant_land_2_{exarch['tag']}_tt\n"
                f"		trigger = {{ nhs_{exarch['tag']}_merchant_land_2 = yes }}\n"
                f"	}}\n")
