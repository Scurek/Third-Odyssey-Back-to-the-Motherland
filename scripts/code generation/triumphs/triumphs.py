...
output_file_scripted_triggers = "o_scripted_triggers.txt"
...
template_events = Template('''{%- for region in regions %}
to_has_completed_triumph_{{ region.region }} = {
    check_variable = {
		which = to_{{ region.region }}_provinces
...
f.write(f"# Following set of scripted triggers was generated by {os.path.basename(os.getcwd())}"
        f"/{os.path.basename(__file__)}")
f.write(template_events.render(regions=regions))
f.close()