...
PixelShader = 
{
	...
	[[
		...
		{
            float dif = CurrentState - v.vTexCoord0.x;
            if (dif < 0.006 && dif >= 0) {
                return float4(0, 0, 0, 1);
            }
            else if (dif > 0) {
                return (1 - CurrentState) * tex2D( TextureOne, v.vTexCoord0.xy ) + CurrentState * tex2D( TextureTwo, v.vTexCoord0.xy );
                // return tex2D( TextureOne, v.vTexCoord0.xy );
            }
            else {
                return float4(0.63, 0.63, 0.63, 0.99);
            }
		}
	]]
